...
\begin{enumerate}
	\item Mit $f_*$: kann man ein Wort $w \in X^*$ (und ggf. einen Zustand) eingeben und erhält den Endzustand
	\item Mit $f_{**}$ kann man ein Wort eingeben und erhält alle durchlaufenen Zustände
	\item Mit $g_*$ kann für ein Wort die letzte Ausgabe angegeben werden
	\item Mit $g_{**}$ kann für ein Wort alle Ausgaben angegeben werden
\end{enumerate}
\ \\
...
\ \\
Auch beim Moore-Automaten gelten die 4 Funktionen, $f_*$, $f_{**}$ sind identisch, aber $h_*$, $h_{**}$ ersetzten $g_*$, $g_{**}$.\\
...
\ \\
\begin{itemize}
	\item $f_*(z_1, STSTST) = z_3$
	\item $f_*(z_2, TTTSSS) = z_2$
	\item $h_{**}(SSTSTT) = abacabb$
	\item $h_{**}(z_1, \epsilon) = b$
	\item $h_*(z_3, TSSSSS) = a$
	\item $h_*(z_1, STST) = b$
\end{itemize}
\end{frame}
...
Mathematisch: $f_*(w) \in F \Leftrightarrow$ \emph{"EA akzeptiert w"} \\
\ \\
