...
\ \\
\begin{itemize}
	\item $f(n) = \Pi n^{10} \leq ( e^9 \Pi ) \cdot \frac{1}{e^9} n^{10} = e^9 \Pi = g(n)$ ab $n_0 = 1$
	\begin{itemize}
		\item $f(n) \in O( g(n) )$
	\end{itemize}
	\item $g(n) = \frac{1}{e^9} \leq n^{10} \leq \Pi n^10 = c \cdot f(n)$
	\begin{itemize}
		\item $g(n) \in O( f(n) )$
	\end{itemize}
\end{itemize}
...
Die Lösung steht in einem Extra-PDF.
\end{frame}
...
\begin{enumerate}
	...
	\item Ist $f(n) \in \Omega( n^{{log}_b a+\epsilon} )$ für ein $\epsilon > 0$ und $\exists d \in (0,1)$, sodass für ein großes n gilt: $a \cdot f(\frac{n}{b}) \leq d f(n) \Rightarrow T(n) \in \Theta( f(n) )$
\end{enumerate}
\end{frame}
